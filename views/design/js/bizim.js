...
$(document).ready(function(e) {
  ...
  // siparişi tamamladaki Farklı Bilgiler Kullanı seçince textlerin içini temizleme
  var ad,soyad,mail,telefon;
  $('input[name=bilgiTercih]').on('change',function() {	
    
    var gelenTercih=$(this).val(); // 0(kendi bilgileri) - 1(başka bilgiler)
    
		if (gelenTercih==1) 		
		{
      // textlerin içindeki veriyi yakalar
			ad=$('input[id=sipAd]').val();
			soyad=$('input[id=sipSoyad]').val();
			mail=$('input[id=sipMail]').val();
			telefon=$('input[id=sipTlf]').val();
			
			// textlerin içindeki veriyi siler
      $('input[id=sipAd]').val("");
      ...
      $('input[id=sipTlf]').val("");
			...
			// textlerin eski değerlerini verir
